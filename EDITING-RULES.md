...
17. `[FIELD]` **Never compute a video crop from ffprobe's width/height in Python. Build it from iw/ih inside ffmpeg.** Phone clips carry a rotation side-data tag: ffprobe reports the STORED raster (3840x2160) while ffmpeg decodes the DISPLAYED frame (2160x3840). A Python crop built on the stored size slices a narrow strip out of the wrong axis, and the render still comes out at exactly 1080x1920, so no size check catches it. It shipped twice (2026-09-09 to a paid delivery, 2026-09-23 to IG/FB): ten seconds of a cat's back with no face. `templates/framing.py` fixes it structurally: `crop_to(9,16,cx,cy)` is an ffmpeg expression measured on the decoded frame and collapses to identity when the source is already the target aspect (a portrait clip going to 9:16 is scale-only); `decoded_size()` is the only correct way to ask a clip's size from Python; `shot_sheet(segs, png)` renders first/middle/last frame of every segment. Open the sheet and look at every row before compositing. Every framing failure so far was visible in that sheet and invisible in every numeric check.
...
27. Framing: every crop built with `framing.crop_to` (iw/ih inside ffmpeg, never from ffprobe numbers in Python), and the per-segment shot sheet rendered AND looked at, every row, before the overlay pass?
