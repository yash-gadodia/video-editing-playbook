...
16. `[FIELD]` **A gag tag needs a home that is not a face.** `#NOTSPONSORED` slapped top-right landed squarely on someone's hair. Below the subtitle, rotated a few degrees, it reads as a cheeky footnote to the line above it - which is funnier anyway, because it is annotating the claim.
