...
| **Crop inside ffmpeg (`templates/framing.py` `crop_to`), never from ffprobe numbers in Python, and LOOK at the per-segment shot sheet before compositing.** | Phone clips decode rotated (2160x3840) while ffprobe reports 3840x2160; a Python crop slices the wrong axis and still renders at exactly 1080x1920. Shipped a faceless cat reel twice. EDITING-RULES 12.17. |
